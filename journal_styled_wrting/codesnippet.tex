...
\Abstract{In this paper we present a package implementing the results of the paper by Turner et. al.  In particular, we wrote codes to compute the persistent homology transform (PHT) of objects in $\mathbb{R}^3$ and shapes in $\mathbb{R}^2$.  PHT is a statistic that completely describes a shape or surface using topological properties, and thus allows us to determine a metric on the space of piecewise linear shapes. A metric opens up many different possibilities for statistical analysis, including clustering. }
